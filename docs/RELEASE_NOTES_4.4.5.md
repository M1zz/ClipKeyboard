...
(앱 안 변경 이력은 `ChangelogView.swift` 의 `4.4.5` 항목, 문구가 서로 어긋나지 않게 함께 고칠 것)

> ⚠️ **4.4.4 를 승계한다.** 4.4.4 는 App Store 에 나가지 않았다(`ChangelogView` 의 `released: nil`).
> 사용자는 **4.4.3 에서 곧장 넘어오므로**, 아래 문안에는 4.4.4 내용이 그대로 들어 있다.
> `docs/RELEASE_NOTES_4.4.4.md` 는 기록용으로 남겨 둔다. 심사에 올릴 문안은 이 파일이다.
...
- 4.4.4 의 심사 메모(붙여넣기 프롬프트 시점, 키보드 감지, 스킨 비노출, 자산 감소, 튜토리얼 상태 키,
  **인도네시아어 정리 미완**, macOS 앱 호환성)는 **전부 그대로 유효하다**
  `docs/RELEASE_NOTES_4.4.4.md` 의 같은 절을 볼 것. 4.4.4 가 안 나갔으므로 이번에 함께 나간다.
