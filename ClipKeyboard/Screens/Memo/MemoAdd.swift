...
struct MemoAdd: View {
    ...
    /// 초기화와 키보드 내리기. **글 쓰는 자리를 넓히려고 위로 올린 것들이다.**
    ///
    /// ⚠️ 둘 다 글자 없이 기호만 세운다. 아래 줄에 있을 때는 이름표가 필요했지만
    ///    머리 줄에서는 자리가 좁고, 무엇인지는 VoiceOver 라벨이 말한다.
    /// ⚠️ 초기화를 바깥 끝에 두지 않는다. 지우는 일이라 엄지가 스치기 쉬운 자리에
    ///    두면 안 된다. 바깥 끝은 눌러도 아무것도 잃지 않는 키보드 내리기가 맡는다.
    ...
    private var formToolbar: some ToolbarContent {
        ToolbarItemGroup(placement: .navigationBarTrailing) {
            ...
            if isFocused || isTitleFocused {
                ...
                .accessibilityLabel(NSLocalizedString("키보드 내리기", comment: "Dismiss keyboard button"))
            }
        }
    }
        ...
        VStack(spacing: 0) {
            ...
            ScrollViewReader { proxy in
                ...
                VStack(spacing: 0) {
                    ...
                    // 아래에는 **저장 하나만** 둔다.
                    //
                    // ⚠️ 예전에는 이 줄에 초기화와 저장이 나란히 있었고, 글을 쓰는 동안에는
                    //    저장 자리에 '완료'(키보드 내리기)가 들어섰다. 그래서 정작 저장하려면
                    //    키보드를 먼저 내려야 했다. 둘 다 머리 줄로 올렸다(`formToolbar`).
                    //    남은 한 칸이 넓어지고, 쓰는 도중에도 바로 저장할 수 있다.
                    Button {
