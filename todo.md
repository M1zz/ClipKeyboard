...
- [x] 앱에 닿는 길 36개를 6묶음으로 전수 나열: 측정(따로·섞여·못 셈) · 상태 · 어디서 보나 · 평가 지표 · 근거 문서
- [x] 알게 됨 → Pro 7칸 흐름, 경로 이름표가 끊기는 자리(설치 뒤) 표시
- [x] 재는 도구 표, 비교 기준, 지금 빈칸 5개
- [ ] 캠페인 토큰 pt 발급 후 index·tutorial 링크 6곳과 앱 안 공유 링크에 ct 달기
- [ ] Search Console 등록 여부 확인해서 페이지에 반영
- [x] "앱 문제인가, 홍보 문제인가" 절: 세 구역, 숫자별 확인 가능 여부·방법, 가리는 순서
- [ ] ASC 에서 노출·조회·전환율·첫 다운로드·벤치마크를 읽어 페이지 "옮기지 않음" 칸 채우기
- [x] 스냅샷에 flag.keyboardEnabled · keyboardPastes · state.level 추가, 통계 화면 "키보드에 닿기까지"(최근 30일/전체) + ActivationReportTests
- [ ] 기기에서 확인: 전체 접근 끈 채 키보드를 띄우면 kb.beacon.lastUse 가 App Group 에 남는지 (안 남으면 7.2% 는 과소)
- [ ] 다음 버전 배포 뒤 2~4주: 키보드에 닿기까지 최근 30일로 읽고 가장 크게 빠지는 칸 정하기
- [ ] 페이월을 본 설치 5/5,756: 결제 문이 서는 조건 점검
...
결제가 말이 되는 순간을 셋 더 붙잡는다.

### 상품

- [x] **칸 추가를 다시 살 수 있게** - 소모성 `slots5.pack` 신설, 최대 세 번(15칸).
      예전 비소모성 `slots5` 는 이미 산 사람의 권한으로만 남는다
      (App Store Connect 는 올린 상품의 종류를 못 바꾼다)
- [x] 소모성은 복원이 안 되므로 iCloud 키·값에 팩 수를 같이 새긴다. 둘 중 **큰 값**을 쓴다
- [x] **업그레이드 값** `pro.upgrade` - 칸을 산 사람에게만 보이는 평생 값.
      `entitlementIDs` 에 넣는다(안 넣으면 돈만 받는 상품이 된다)
- [x] **두 대째** `twodevice` - 동기화만 연다. Pro 가 아니다.
      `ProFeatureManager.isSyncAvailable` 이 동기화를 묻는 단일 출처
- [x] 설치 직후(`firstRun`) 반값 폐지. 가치를 보기 전의 할인은 정가만 무너뜨린다

### 결제가 말이 되는 순간 (`PurchaseMomentManager`)

- [x] 가려야 할 것을 저장한 직후(24시간 안) → 생체잠금
- [x] 두 번째 기기가 나타났을 때 → 두 대째
- [x] 쌓인 것 20개 + 백업 없음 + 설치 30일 → 평생(백업)
- [x] 대량 가져오기에서 **몇 개가 안 보이게 되는지 숫자로** 말한다
- [x] 순간은 각각 평생 한 번. 배너가 뜨는 그 자리에서 못박는다

### 화면

- [x] 페이월 머리에 "지금 내 단축어 n / m칸"
- [x] 칸 추가 버튼 조건을 "안 샀나" 에서 "**더 살 수 있나**" 로
- [x] 업그레이드 값일 때 왜 싼지 한 줄로

### 측정

- [x] `prior_purchase` (none · slots · two_device · pro) - **결제 전에** 찍는다.
      끝난 뒤에 물으면 모두가 "뭔가 산 사람" 이 되어 사다리 가설이 검증 불가능해진다

### 남은 것

- [ ] App Store Connect 에 상품 셋 등록 (`pro.upgrade` 비소모성 · `slots5.pack` **소모성** · `twodevice` 비소모성)
- [ ] 등록 전까지 그 상품들은 로드되지 않는다. 화면은 상품이 없으면 버튼을 안 그린다(의도한 동작)
- [ ] `SlotPack.swift` 주석의 값과 App Store Connect 실제 값 맞추기

## ✨ 새로 만든 것이 어디에 생겼는지 보이게 (2026-09-04)

사용자 요청. "지금 추가를 하면 너무 뷰가 바로 생기는데 (...) 빈 것을 생성하고 나서
1초 뒤에 그 안에 내용을 채우는 것과 같은 느낌을."

저장하면 완성된 카드가 격자 어딘가에 툭 나타났다. 그게 어디에 생겼는지 눈이 못 쫓는다.
빈 자리가 먼저 서면 눈이 거기로 가고, 그 다음 내용이 들어오는 것을 **본다.**
같은 결과지만 어디에 무엇이 생겼는지가 남는다.

- [x] `NewMemoIntro` - `.memoSaved` 를 스스로 받는다. 화면마다 이어 붙이지 않는다
      (그러면 새 화면을 만들 때마다 잊는다)
- [x] `.newMemoIntro(memo.id)` 한 줄로 붙인다. 목록 카드(`MemoCardSurface`)와
      키보드 키 셋(글자·이미지·콤보)
- [x] **바탕이 아니라 내용에만 건다.** 카드의 면과 키캡은 그대로 서 있어야 한다.
      거기까지 사라지면 "빈 자리가 생겼다" 가 아니라 "아무 일도 없었다" 가 된다
- [x] 시험 5개

### 부드럽게 (요청: "부드러운 애니메이션이 들어갔으면 해")

- [x] **자리가 자라며 들어온다** (`NewMemoIntroEntry`, 0.88 → 1 스프링).
      이게 없으면 빈 카드가 툭 나타난 뒤에야 부드러워져서 연출의 첫 프레임이 제일 거칠다.
      **방금 만든 그 하나만** 움직인다 - 나타나는 모든 카드에 걸면 탭을 옮길 때마다
      격자 전체가 피어나고, 그러면 새로 생긴 것이 어디인지는 오히려 안 보인다
- [x] **뼈대가 숨을 쉰다.** 1초는 짧지 않아서, 아무 일도 안 일어나면 비어 있는 것이
      연출이 아니라 고장으로 읽힌다
- [x] 들어올 때 스프링(response 0.55). 살짝 작게·살짝 아래에서·살짝 흐리게 올라온다.
      셋 다 아주 조금씩이다 - 크게 주면 연출이 눈에 띄어 정작 무엇이 생겼는지를 가린다
- [x] 움직임 줄이기를 켠 사람에게는 숨도 스프링도 없다. 밝기만 부드럽게 바뀐다
- [x] **뼈대를 실제로 그려서 봤다.** 처음에는 카드 맨 위에 붙어 있었는데, 채워진 카드의
      제목은 세로 가운데쯤에 선다. 줄이 위에 있다가 글이 가운데 나타나면 두 동작이 되어
      같은 것이 채워졌다고 안 읽힌다. 세로 가운데로 옮겼다
- [x] 애니메이션을 오버레이보다 **뒤에** 건다. 앞에 걸면 뼈대가 사라지는 것은 그 곡선을
      못 타서 글이 올라오는 동안 뼈대만 툭 없어진다

### 걸릴 뻔한 것 (이게 핵심이다)

**저장 화면은 저장하고 1초 뒤에 닫힌다**(`MemoAddViewModel.saveMemo` 의 asyncAfter).
저장한 순간부터 1초를 세면 그 1초가 **시트 뒤에서 다 지나가고**, 사용자는 여전히
완성된 카드가 툭 나타나는 것만 본다. 요청의 정반대가 된다.

그래서 표시만 걸어 두고 시간은 세지 않는다. 카드가 실제로 화면에 나타났다고
알려 올 때(`noticeAppeared`, 뷰의 `onAppear`) 그때부터 1초를 센다.
끝내 안 나타나면(다른 탭에 생겼거나 화면을 바로 떠났거나) 12초 뒤에 스스로 푼다.
안 풀면 그 단축어는 다음에 열 때도 빈 카드로 서 있다.
