...
final class CategoryPageWindowTests: XCTestCase {
    ...
    func test_창의_한가운데와_좌우_한_장을_짓는다() {
        for index in 4...6 {
    ...
    func test_창_밖은_짓지_않는다() {
        XCTAssertFalse(built(3, center: 5, selected: 5))
        XCTAssertFalse(built(7, center: 5, selected: 5))
    ...
    func test_카테고리가_늘어도_짓는_페이지_수는_그대로다() {
        for total in [5, 20, 60] {
            let count = (0..<total).filter { built($0, center: 2, selected: 2) }.count
            XCTAssertEqual(count, 3, "카테고리 \(total)개에서도 세 장만 짓는다")
        }
    }

    /// 창이 못 따라온 사이에는 한 장이 더 설 수 있다. 그 이상은 아니다.
    func test_창이_뒤처져도_최대_네_장이다() {
        let count = (0..<60).filter { built($0, center: 5, selected: 20) }.count
        XCTAssertEqual(count, 4, "창 세 장 + 보고 있는 한 장")
    }
}
