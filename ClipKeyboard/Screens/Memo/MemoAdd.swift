...
struct MemoAdd: View {
    ...
    var body: some View {
        ...
        // 아래 저장 버튼을 없앴으므로 **적는 것은 기본으로 남는다.** 화면을 닫을 때만 남기면
        // 앱이 뒤로 간 채 종료될 때 적던 것이 사라진다. 뒤로 갈 때도 한 번 남긴다.
        // (같은 드래프트를 고쳐 쓰므로 여러 번 불려도 한 장이다 - VM `autosaveDraftId`)
        .onChange(of: scenePhase) { _, phase in
            if phase != .active { viewModel.saveDraftIfNeeded() }
        }
    ...
    /// 초기화와 저장(V). **글 쓰는 자리를 넓히려고 위로 올린 것들이다.**
    ///
    /// ⚠️ 아래 줄의 저장 버튼은 없앴다. 적는 것은 기본으로 임시저장되고(`saveDraftIfNeeded`),
    ///    다 적었으면 머리 줄 V 로 넣는다. 키보드가 올라와 있어도 가려지지 않는 자리다.
    /// ⚠️ 키보드 내리기 버튼도 없앴다. 입력칸 바깥을 누르거나 목록을 끌면 내려간다
    ///    (`dismissKeyboard`).
    /// ⚠️ 초기화를 바깥 끝에 두지 않는다. 지우는 일이라 엄지가 스치기 쉬운 자리에
    ///    두면 안 된다. 바깥 끝은 V 가 맡는다.
    ...
    private var formToolbar: some ToolbarContent {
        ToolbarItemGroup(placement: .navigationBarTrailing) {
            ...
            .accessibilityLabel(NSLocalizedString("초기화", comment: "Reset"))
            ...
            .disabled(!canSave)
            .accessibilityLabel(NSLocalizedString("저장", comment: "Save"))
        }
    }
    ...
    /// 입력칸 바깥을 눌렀다. 따로 버튼을 두지 않고 여기서 키보드를 내린다.
    ...
    /// 지울 것이 있는가. 없으면 초기화 버튼을 흐리게 둔다 - 눌러도 아무 일도 안 일어나는
    /// 버튼이 멀쩡히 서 있으면 누른 사람은 앱이 멈춘 줄 안다.
    private var hasAnythingToReset: Bool {
        !viewModel.value.isEmpty || !viewModel.keyword.isEmpty || !viewModel.attachedImages.isEmpty
    }
    ...
    private var addBody: some View {
        VStack(spacing: 0) {
            ...
            ScrollViewReader { proxy in
                ...
                // 입력칸 바깥을 누르면 키보드가 내려간다. 목록을 끌어도 내려간다.
                //
                // ⚠️ 제스처를 **바탕에만** 건다. 본문 전체에 걸면 입력칸을 누르는 손길까지
                //    받아서, 커서가 들어가자마자 다시 빠진다.
                ...
                // 키보드 바로 위에 붙는 줄 - 쓸 때 채우는 칸과 짚어 주는 띠.
                // 저장은 머리 줄 V 로 올라갔다(`formToolbar`). 보여 줄 것이 없으면 줄도 세우지 않는다.
                .safeAreaInset(edge: .bottom, spacing: 0) {
                if isFocused || coachStep != nil {
                VStack(spacing: 0) {
                    Divider()
    ...
    private var titleInputSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            TextField(NSLocalizedString("예: 회사 이메일, 송금 계좌", comment: "Memo title field placeholder"), text: $viewModel.keyword)
                ...
                // 상자 **어디를** 눌러도 커서가 들어간다. 글자 줄만 눌리면 위아래 여백을
                // 누른 사람은 안 눌린 줄 안다.
                .contentShape(Rectangle())
    ...
    private var hintInputSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            TextField(NSLocalizedString("카드에 살며시 보여줄 한 줄, 비우면 자동 요약", comment: "Custom hint field placeholder"),
                ...
                .cornerRadius(theme.radiusMd)
                .contentShape(Rectangle())
                .onTapGesture { isHintFocused = true }
