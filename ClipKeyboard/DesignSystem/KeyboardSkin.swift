...
/// 키캡 윤곽 - 둥근 사각형.
///
/// ⚠️ 여기 있던 "악어 입속"(아래로 좁아지는 송곳니 + 잇몸 + 어두운 입 안쪽)을 걷어냈다.
///    마스코트가 사라진 뒤로 이빨은 가리키는 것이 없었고, 설정에 토글만 남아
///    켜 보면 무엇을 흉내 내는지 알 수 없는 모양이 됐다.
struct KeycapShape: InsettableShape {
    ...
    func inset(by amount: CGFloat) -> KeycapShape {
        KeycapShape(radius: radius, inset: inset + amount)
    }
