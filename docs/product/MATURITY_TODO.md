...
> 설계 배경: `docs/product/SERVICE_MATURITY_PLAN.md` · 이 문서는 그 **실행 목록**이다
...
- [ ] **CloudKit 스키마 Production 배포**, `UsageSnapshot`/`UsageEvent` 생성 → 인덱스(recordName Queryable, createdTimestamp Sortable) → admin read → Deploy.
      절차: `docs/product/APP_PRIVACY_ANSWERS.md` 0절 · **콘솔 수동 작업**
- [ ] **실기기 검증**, 스냅샷 1건 업로드 → `설정 > 지원 > 사용 통계`에 카운트·차트 렌더링 확인.
      지금까지 유닛 테스트·빌드까지만 검증됐고 실데이터 렌더링은 미확인
- [ ] **App Store Connect App Privacy 입력**, 6개 항목. 답안: `docs/product/APP_PRIVACY_ANSWERS.md` 1절 · **콘솔 수동 작업**
...
- [x] **파일 보호 등급 확인·문서화** ✅ `docs/engineering/SECURITY_NOTES.md` 2-1절 (올리지 않는 이유(잠금화면 키보드가 깨진다) 기록) 현재 명시 설정 없음(iOS 기본값).
...
- [ ] **App Store 설명 갱신**, `docs/marketing/ASO_2026-07.md` 기준으로 교체(프라이버시 문단이 수집 사실 반영본으로 바뀌었다)
- [ ] **공개 쇼케이스 사이트 발행**, `docs/index.html` GitHub Pages 배포 확인
- [ ] **커뮤니티 게시**, `docs/marketing/KR_COMMUNITY_POSTS.md` · `docs/marketing/REDDIT_LAUNCH_POST.md`(발행 시 "no data collection" 문구 수정 필요) · `docs/marketing/THREADS_LAUNCH_POST.md`
- [ ] **Product Hunt 런치**, `docs/marketing/PRODUCT_HUNT_LAUNCH.md`
- [ ] **ASO 키워드 반영 및 추적**, `docs/marketing/ASO_2026-07.md` 키워드 적용 후 순위 변화 기록
...
- [x] **최소 권한·암호화 근거 문서화**, `docs/engineering/SECURITY_NOTES.md`
      (키보드·위젯 엔타이틀먼트에 App Group만 있어 유출 경로가 구조적으로 없다는 점 포함)

## 프로세스 항목 (코드 아닌 것)

- [x] **TestFlight 베타 루틴** ✅ `docs/engineering/RELEASE_PROCESS.md` 3절, 릴리즈 전 외부 테스터 N명, 최소 3일
- [x] **회귀 테스트 체크리스트** ✅ `docs/engineering/RELEASE_PROCESS.md` 2절 (자동 테스트가 못 잡는 것만), `docs/engineering/TESTING_GUIDE.md`를 릴리즈 전 필수 통과 목록으로 승격
- [x] **롤백 전략 문서화** ✅ `docs/engineering/RELEASE_PROCESS.md` 4절 (사고 등급별 대응 + 킬스위치 절차 + 긴급 심사) App Store 단계적 출시(phased release) 중단 절차 + 이전 버전 재제출 판단 기준
- [x] **코드 리뷰 프로세스** ✅ `docs/engineering/RELEASE_PROCESS.md` 1절 (CI·SwiftLint·/code-review 를 머지 조건으로) 1인 개발이라 `/code-review` + CI 통과를 머지 조건으로 정의
