...
- 한도 판정도 같은 개수를 센다(`ProFeatureManager.ownMemoCount`). 그래서 이 차트에서
  읽는 거리가 곧 결제 자리까지의 거리다. 예전에는 판정만 샘플을 세어 거리가 4만큼
  어긋나 있었다(v5.0.5 에서 맞췄다). 둘 중 한쪽만 고치면 차트는 다시 거짓말이 된다
