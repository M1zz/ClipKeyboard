...
```
단축어를 복사하면 화면 아래 알림 대신 누른 카드에 바로 복사됨이 표시돼요.
키보드의 카테고리 탭 글자가 흐려 보이던 것을 고쳤어요.
...
```
When you copy a snippet, the card you tapped now says Copied, instead of a banner at the bottom of the screen.
Category tabs on the keyboard no longer look blurry.
...
```
复制短语时，不再在屏幕底部弹出提示，而是直接在你点按的卡片上显示已复制。
修复了键盘分类标签文字看起来模糊的问题。
...
```
複製短語時，不再在螢幕底部跳出提示，而是直接在你點按的卡片上顯示已複製。
修正了鍵盤分類標籤文字看起來模糊的問題。
...
```
При копировании фразы отметка Скопировано теперь появляется прямо на нажатой карточке, а не баннером внизу экрана.
Вкладки категорий на клавиатуре больше не выглядят размытыми.
