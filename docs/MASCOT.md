...
이 칸만 유독 까다롭다. 다른 칸은 그림을 그냥 놓으면 되지만, 여기는 **코드가 그린
껍데기와 한 장면에서 만나야** 하기 때문이다. 두 번 시도했다가 두 번 다 안 맞았다.

- **머리만.** 전신은 안 된다. 그림이 서는 자리는 키 한 칸(약 150×52pt) 위이고
  악어는 74pt로 그려진다. 전신을 넣으면 **몸이 키를 통째로 덮어 껍데기가 안 보인다.**
- **손·팔이 프레임 안에 없어야 한다.** 두 손을 든 포즈는 손이 얼굴 옆에 붙어 있어서,
  머리만 잘라내려 해도 손이 딸려 온다(팔로 이어져 있어 떼어낼 수도 없다).
- **옆모습, 주둥이는 왼쪽.** 껍데기는 장면 한가운데 있고 악어는 오른쪽에서 들어온다.
  앞모습이면 입이 어디를 무는지 알 수 없다.
- **입은 비어 있어야 한다.** 물릴 것은 코드가 그려서 입 안에 놓는다.
- 입이 그림의 **아래쪽·왼쪽**에 오면 가장 좋다. 위에서 내려다보며 무는 자세가 된다.

그림이 들어오면 `ShellCrackOverlay.crocRest` 를 그 그림의 입 위치에 맞춰 다시 잡는다
(계산법은 그 상수 옆에 적어 두었다). 안 고치면 **허공을 문다.**
