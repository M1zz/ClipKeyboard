...
- ⚠️ **앞 버전(5.0.6)이 스토어에 나갔는지 먼저 확인할 것.** 이 문안은 5.0.6 이후에 바뀐 것만
  담았다. 5.0.6 이 심사에 안 올라갔거나 반려된 채로 남아 있다면 사용자는 5.0.5 에서 곧장
  넘어오므로, `5.0.6.md` 의 문안(날짜·시간 모양 고르기, 저장할 때 카테고리 정하기, 키보드
  높이를 시스템에 맞춘 것, 카테고리가 저절로 늘지 않게 한 것, 목록 번쩍임)을 이 문안 **앞에**
  붙여야 한다. 그 경우 5.0.6 쪽을 세 문단으로 줄여 얹는다.
  `5.0.6.md` 의 심사 메모에 적힌 CloudKit 스키마 작업(`CrashReport` 필드 5개 추가 +
  Production 배포)과 실기기 확인 목록도 그대로 살아 있다.
...
- 앱 안 변경 이력(`ChangelogView`)의 옛 항목은 여전히 "플레이스홀더"라고 적혀 있다.
  ...
  안 이어질 수 있다.
- `ChangelogView.swift` 에 **5.0.6 항목이 없다.** 5.0.5 다음이 곧장 5.0.7 이다.
  5.0.6 이 스토어에 나갔는지 정한 뒤에 함께 결정할 것 - 안 나갔으면 지금이 맞고,
  나갔으면 5.0.6 항목을 채워 넣어야 앱 안 기록에 구멍이 안 남는다.
