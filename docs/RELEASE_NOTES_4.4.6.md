...
(앱 안 변경 이력은 `ChangelogView.swift` 의 `4.4.6` 항목, 문구가 서로 어긋나지 않게 함께 고칠 것)

> ⚠️ **4.4.5 와 4.4.4 를 함께 승계한다.** 둘 다 App Store 에 나가지 않았다(`ChangelogView` 의 `released: nil`).
> 사용자는 **4.4.3 에서 곧장 넘어오므로**, 아래 문안에는 4.4.4 · 4.4.5 내용이 그대로 들어 있다.
> `docs/RELEASE_NOTES_4.4.4.md` 와 `docs/RELEASE_NOTES_4.4.5.md` 는 기록용으로 남겨 둔다.
> 심사에 올릴 문안은 이 파일이다.
>
> 4.4.6 에서 새로 더해진 것은 **설정 화면 정리**와 **안정성 화면 문구**뿐이다. 아래에서 각각
> "설정이 8개 묶음으로 정리됐어요" · "Settings, now eight groups" 절과 고친 것 목록의 마지막 줄이다.
...
• 예전에는 보관함에 담아 두고 나중에 앱에서 "단축어로 만들기"를 눌러야 했어요. 이제 **단축어가 기본**이고, 나중에 정하고 싶으면 "보관함"을 고르면 됩니다.
...
• 고를 수 있는 건 **즐겨찾기한 단축어**예요. 보안 단축어는 이 경로에 올라오지 않습니다.
...
- **버전**: `Version.xcconfig` 의 `MARKETING_VERSION = 4.4.6`. 빌드 번호는 업로드 전 올릴 것.
...
- 4.4.4 의 심사 메모(붙여넣기 프롬프트 시점, 키보드 감지, 스킨 비노출, 자산 감소, 튜토리얼 상태 키,
  **인도네시아어 정리 미완**, macOS 앱 호환성)는 **전부 그대로 유효하다**
  ...
  4.4.4 · 4.4.5 가 안 나갔으므로 이번에 함께 나간다.
...
자세한 배경은 `docs/USAGE_STATS_HUB.md`.
...
관련해 다음 버전에 넣을 것(이번 4.4.6 범위 아님):
...
### 🩺 안정성 화면은 스키마가 배포돼야 제 문구가 나온다 (4.4.6 신규)

설정 > 지원 > 안정성 이 `CrashReport` 레코드 타입을 조회한다. 그런데 CloudKit 은 **저장이
성공할 때만**, 그것도 Development 환경에서만 레코드 타입을 만든다. 조회로는 만들어지지 않는다.
이 타입에 쓰는 유일한 경로인 `DiagnosticsService.upload` 는 실기기 + 실제 크래시 + 원격 플래그가
모두 맞아야 도는 탓에 **한 번도 성공한 적이 없다.** 수집 코드와 화면만 있고 스키마가 없는 상태다.

4.4.6 은 그 상태를 서버 원문 대신 한국어 안내로 바꾼다(`CrashReportReader.isSchemaNotReady` 가
`unknownItem` · `invalidArguments` 를 묶어 판정). 즉 **미배포여도 사용자에게 고장으로 보이지는
않는다.** 다만 화면이 제 일을 하려면 배포가 필요하다.

- 절차(필드 7개 + 인덱스 2개 + Security Roles + Production 배포): `docs/CRASH_REPORT_SCHEMA.md`
- ⚠️ `_world` 에 read 를 주지 말 것. 콜스택은 admin 역할만 읽는다(피드백 · 통계와 같은 규약).
...
(`QuickNoteControl` 의 kind 가 v4 까지 간 이유). 상세: `docs/CONTROL_CENTER_APP_LAUNCH.md`
