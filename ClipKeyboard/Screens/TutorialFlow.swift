...
struct TutorialWelcomeView: View {
    ...
    /// 맞이하는 기호. 브랜드색 옅은 원 위에 손 흔드는 기호를 얹는다.
    ///
    /// ⚠️ 여기 있던 손 흔드는 악어 영상(`MascotWave.mov`)은 캐릭터를 걷어낼 때 지웠다.
    ///    번들에서 파일을 찾아 재생하던 코드가 남아 있어서, 증분 빌드로 옛 영상이 남은
    ///    기기에서는 악어가 계속 떴다. 파일을 찾는 길 자체를 없앴다.
