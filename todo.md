...
- [ ] ⚠️ **한도 판정은 아직 샘플을 센다.** `canAddMemo(currentCount: existingMemos.count)`
      (`MemoAddViewModel:544`·`:634`)가 전체 개수를 본다. 무료 사용자는 아무것도
      안 만들었는데 4/10 에서 시작하고, **실질 무료 칸은 6개**다. `freeMemoLimit = 10` 에
      "가치 검증 시간 확보를 위해 v4.1에서 한도 상향" 이라고 적혀 있는데 그때 올린 4칸이
      샘플로 도로 메워져 있다. 차트와 페이월이 다른 숫자를 보는 동안 거리가 4만큼 어긋난다
      (차트 아래 경고로 일단 밝혀 둠). 고치면 실질 무료 칸이 6 → 10 이 되므로 **수익 판단**이다
- [ ] `DiscountOfferManager.limitEdgeCount`(9개)도 같은 값을 본다. 지금 할인 제안은
      자기 걸 5개 만든 사람에게 나간다
- [ ] 빌드·시험으로 확인 못 함. 작업 중인 `LeeoCloudPage`(LeeoKit 3.3.0)가 원격에
      아직 태그가 없어(최신 v3.2.0) 저장소 전체가 컴파일되지 않는다. 구문 검사만 통과
