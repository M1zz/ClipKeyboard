...
enum ShellCrack {
    ...
    // MARK: - 시간표
    //
    // ⚠️ 이 값들은 `ShellCrackOverlay.run()` 의 밀리초와 **짝이 맞아야 한다.**
    //    장면은 저기서 그리고, 값이 언제 들어갈지는 여기를 보고 정한다.
    //    한쪽만 고치면 악어가 깨물기도 전에 글이 들어가 있거나, 다 끝난 뒤에 들어온다.

    /// 껍데기가 갈라져 **알맹이가 드러나는** 순간(초).
    ///
    /// 값은 이때 들어가야 한다. 이보다 빠르면 깨기도 전에 이미 나와 있고, 그러면
    /// 이 장면이 무엇을 설명하는 그림인지 알 수 없다.
    static let revealDelay: Double = 0.34

    /// 악어가 화면 밖으로 **다 나간** 시각(초).
    ///
    /// 템플릿 빈칸 시트처럼 화면을 덮는 것은 이 뒤에 연다. 먼저 열면 악어가 그 뒤에서
    /// 혼자 깨물고 사라진다 - 사용자는 아무것도 못 본다.
    static let clearedDelay: Double = 0.9
