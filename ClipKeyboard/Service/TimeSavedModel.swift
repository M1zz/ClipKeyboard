...
enum TimeSavedModel {
    ...
    ///
    /// ⚠️ **이 모델에서 유일하게 가정이 아닌 값이다.** 나머지 상수는 전부 일어나지 않은
    ///    세상의 시간이라 잰 적이 없지만, 치는 속도만은 관측할 수 있다. 사용자가 문구를
    ///    만들 때 그 값을 우리 편집기에 직접 쳐 넣기 때문이다(`TypingSpeedMeter`).
    ///    그래서 잰 것이 쌓이면 **그 사람의 속도**를 쓰고, 없으면 아래 가정으로 돌아간다.
    ///
    /// 가정의 근거: 모바일 화면 자판의 일반적인 입력 속도는 분당 30~40단어 언저리이고,
    /// 한글은 자모 조합 때문에 이보다 느리다. 한·영 섞어 쓰는 것을 감안해 초당 4자.
    static var proseCharsPerSecond: Double { TypingSpeedMeter.charsPerSecond }
