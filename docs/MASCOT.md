...
|---|---|---|
| `MascotPointing` | 한 손으로 화면 아래(또는 옆)를 **가리키는** 자세 | "여기를 눌러보세요"인데 정면 얼굴이면 어디를 보라는 건지 알 수 없다 |
| `MascotTyping` | 키보드 앞에 앉아 **두드리는** 자세 | 키보드를 켜라는 안내인데 키보드가 그림에 없다 |
| `MascotGuarding` | 팔짱을 끼거나 자물쇠·방패를 **안고 선** 자세 | 보안 단축어 안내에서 웃는 얼굴은 톤이 어긋난다 |
| `MascotBiting` | 무언가를 **깨물어 여는** 옆모습 | 값을 꺼내는 장면의 주인공이라 동작 자체가 그림이다 |
| `MascotApologizing` | 뒤통수를 긁으며 **머쓱해하는** 자세 | 실패 안내에서 활짝 웃고 있으면 놀리는 것처럼 읽힌다 |

넷째 `MascotBiting` 은 `ShellCrack` 이 이름만 같이 쓰므로, 넣을 때 그 화면도 함께 본다.
